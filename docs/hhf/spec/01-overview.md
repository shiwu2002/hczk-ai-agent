...
- **CLI-Anything 工具市场**：平台同步 CLI 工具元数据（registry.json + SKILL.md），管理员在市场启用后，智能体通过系统 Skill "CLI工具市场"（cli_tools_list / cli_tools_install）发现和按需安装
- **用户身份传递**：需要用户隔离的工具（如 knowledge_search），智能体必须在调用参数中携带 `user_id`，平台不自动注入

> 版本：7.0.0 | 日期：2026-06-22
>
> **v7.0.0 变更**（相对 v6.0.0）：
> - 新增 CLI-Anything 工具市场：管理员启用/禁用 CLI 工具，智能体通过"CLI工具市场" Skill 发现和安装
> - CLI 同步改为手动触发，不再自动定时同步
>
> **v6.0.0 变更**（相对 v5.0.0）：
