...
import { Wallet, Zap, CreditCard, Receipt } from 'lucide-vue-next'
      ...
      <div class="glass-card p-6">
        ...
        <div class="flex gap-2 mb-6">
          <button
            ...
            :class="['px-4 py-1.5 text-sm rounded-full transition-all', typeFilter === opt.value ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-white/5 text-slate-400 border border-white/5 hover:bg-white/10']"
          >
            {{ opt.label }}
          </button>
        </div>
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="divide-y divide-white/5">
              <tr v-for="bill in filteredBills" :key="bill.id" class="hover:bg-white/5 transition-colors">
                <td class="py-4">
